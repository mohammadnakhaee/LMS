...
namespace EISProjects
{
    public partial class FormFitting : Form
    {
        ...
        public static int FitSelectedSession=-1;
        public static int FitSelectedCircuit=-1;
        public static int FitSelectedElement=-1;
        public static int FitSelectedMethod=-1;
        ...
        public static bool ChangingCircuitCompleted = true;
        public static bool ChangingSessionCompleted = true;
        Element DeletedElement = new Element();

        //fortran code is in "C:\Users\Mohamad\Documents\Fortran Projects\testcb\bin\Debug\testcb.dll"
        [DllImport(@"..\testcb.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "graphimpedancereim_")]
        ...
        [DllImport(@"..\testcb.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "findfit_")]
        ...
        isMove = false;
        FitSelectedSession=-1;
        FitSelectedCircuit=-1;
        ...
        ChangingCircuitCompleted = true;
        ChangingSessionCompleted = true;
        }
        ...
        private void SessionName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ChangingSessionCompleted)
            {
                FitSelectedSession = SessionName.SelectedIndex;
                ChangingSessionCompleted = false;
                if (SessionName.SelectedIndex < 0)
                {
                    GBContents.Enabled = false;
                    GBElement.Enabled = false;
                    GBFitting.Enabled = false;
                    toolStrip1.Enabled = false;
                    ClearPlot(chart1);
                    foreach (Panel E in ElementsGUI)
                    {
                        E.Dispose();
                    }
                    ElementsGUI.Clear();
                ...
                {
                    if (Form1.AllSessions[FitSelectedSession].Mode != 0)
                    {
                        GBFitting.Enabled = false;
                        LabelError.ForeColor = Color.Red;
                        LabelError.Text = "The mode of this session is not EIS.";
                    }
                    else
                    {
                    ...
                    if (Form1.AllSessions[FitSelectedSession].Mode == 0)
                        ...
                        GBContents.Enabled = false;
                    LoadCircuitList(Form1.AllSessions[FitSelectedSession]);
                    if (Form1.AllSessions[FitSelectedSession].nCircuits > 0)
        ...
        private void LoadCircuitList(Sessions S)
        {
            ...
            CBCircuitName.Items.Clear();
            foreach (Circuit C in S.Circuits)
            {
                CBCircuitName.Items.Add(C.name);
            }

            if (S.nCircuits > 0)
            {
                FitSelectedCircuit = 0;
                CBCircuitName.SelectedIndex = 0;
            }
            else
            {
                FitSelectedCircuit = -1;
        ...
        private void PlotImpedanceExp(Chart chart, double[] FrqArray, double[] ReZArray, double[] ImZArray, int npoints)
        {
            ClearPlot(chart);
        ...
        private void PlotIV(Chart chart, double[] Vlt, double[] Amp, int npoints, bool isLogX, string XAxisName, string YAxisName)
        {
            ClearPlot(chart);
            if (npoints > 0 && isLogX)
                chart.ChartAreas[0].AxisX.IsLogarithmic = true;
            ...
            for (int i = 0; i < npoints; i++)
            {
                AddPoint(chart, 0, Vlt[i], Amp[i]);
            }
        }

        private void PlotImpedanceFit(Chart chart, double[] FrqArray, double[] FitReZArray, double[] FitImZArray, int npoints)
        {
            AddSeries(chart, "Circuit Re(Z)", "Frequancy", "Impedance", Color.Blue);
            AddSeries(chart, "Circuit Im(Z)", "Frequancy", "Impedance", Color.Red);
            chart.Series[0].BorderWidth = 3;
            chart.Series[0].BorderDashStyle = System.Windows.Forms.DataVisualization.Charting.ChartDashStyle.Dot;
            chart.Series[1].BorderWidth = 3;
            chart.Series[1].BorderDashStyle = System.Windows.Forms.DataVisualization.Charting.ChartDashStyle.Dot;
            for (int i = 0; i < npoints; i++)
            {
                AddPoint(chart, 0, FrqArray[i], FitReZArray[i]);
                AddPoint(chart, 1, FrqArray[i], FitImZArray[i]);
            }
            chart.ChartAreas[0].AxisX.IsLogarithmic = true;
        }

        private void PlotImpedanceExpFit(Chart chart, double[] FrqArray, double[] ReZArray, double[] ImZArray, double[] FitReZArray, double[] FitImZArray, int npoints)
        {
            ClearPlot(chart);
            AddSeries(chart, "Re(Z)", "Frequancy", "Impedance", Color.Blue);
            AddSeries(chart, "Im(Z)", "Frequancy", "Impedance", Color.Red);
        ...
        private void PlotFittedImpedance0(Chart chart, double[] FrqArray, double[] ReZArray, double[] ImZArray, double[] FitReZArray, double[] FitImZArray, int npoints)
        {
            ClearPlot(chart);
            ...
            if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].isReadyToFit)
            {
                chart.ChartAreas[0].AxisX.IsLogarithmic = true;
        ...
        private void C_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (isMove)
            {
                ...
                NewLocation.X = NewLocation.X / 6;
                NewLocation.X = NewLocation.X * 6;
                NewLocation.Y = NewLocation.Y / 6;
                NewLocation.Y = NewLocation.Y * 6;
                if (NewLocation.X < 12) NewLocation.X = 12;
        ...
        private void C_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                if ((ModifierKeys & Keys.Control) == Keys.Control)
                {
                    ...
                    {
                        ind++;
                        if (E.HashCode == thisPanel.GetHashCode()) break;
                    }

                    //FitSelectedElement = ind;

                    PointF targetPoint = new Point(e.X, e.Y);
                    if (targetPoint.X < thisPanel.Size.Width / 2)
                    {
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == -1)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.rselinput;
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == -2)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.lseloutput;
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == 1)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.lselres;
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == 2)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.lselcap;
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == 3)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.lselind;
                    }
                    else
                    {
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == -1)
                        ...
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == -2)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.lseloutput;
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == 1)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.rselres;
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == 2)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.rselcap;
                        if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements[ind].Mode == 3)
                            thisPanel.BackgroundImage = global::EISProjects.Properties.Resources.rselind;
                    }
                    thisPanel.Refresh();
        ...
        {
            Panel thisPanel = (Panel)sender;
            SellectElement(Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit], ElementsGUI[FitSelectedElement]);
        }
        ...
        private void BtnNewCircuit_Click(object sender, EventArgs e)
        {
            ...
            CBFitMethod.SelectedIndex = 0;
                ...
                int Y = CircuitDeskTop.Size.Height / 2;
                Y = Y / 6;
                Y = Y * 6;
                input.Location = new Point(96, Y);
                ...
                int X = CircuitDeskTop.Size.Width - 100 - 41;
                X = X / 6;
                X = X * 6;
                output.Location = new Point(X, Y);
        ...
        {
            if (ChangingCircuitCompleted)
            {
                ChangingCircuitCompleted = false;
                foreach (Panel EGUI in ElementsGUI)
                {
                    EGUI.Dispose();
                }
                ...
                int nE = 0;
                Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].nElements = 0;
                foreach (Element E in Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].Elements)
                {
                    nE++;
                    ...
                    //ElementAdded = true;
                    //if (E.Mode == -1 || E.Mode == -2) ElementAdded = false;
                    RearrangeElementsIO(Form1.AllSessions[s].Circuits[c]);
                    ElementAdded = false;
        ...
        private void CircuitDeskTop_Click(object sender, EventArgs e)
        {
            ...
            if (eM.Button == MouseButtons.Left && !TSBSelMod.Checked && !((ModifierKeys & Keys.Control) == Keys.Control))
            {
                Panel NewRes = new Panel();
                NewRes.Parent = CircuitDeskTop;
                Point NewLocation = new Point(eM.X - 20, eM.Y - 10);
                NewLocation.X = NewLocation.X / 6;
                NewLocation.X = NewLocation.X * 6;
                NewLocation.Y = NewLocation.Y / 6;
                NewLocation.Y = NewLocation.Y * 6;
                if (NewLocation.X < 12) NewLocation.X = 12;
                if (NewLocation.Y < 0) NewLocation.Y = 0;
                NewRes.Location = new Point(NewLocation.X, NewLocation.Y);
                NewRes.Size = new System.Drawing.Size(41, 21);
                NewRes.AllowDrop = true;
        ...
        {
            if (FitSelectedSession > -1)
            {
                if (EIS.nSsn > 0 && Form1.AllSessions[FitSelectedSession].nCircuits > 0 && FitSelectedCircuit > -1)
                {
        ...
        private void DrawWires(Circuit C, PaintEventArgs e)
        {
            ...
            if (FitSelectedSession > -1)
            {
                foreach (Link link in C.Links)
                {
                    ...
                    if (sr && er)
                    {
                        Point MidPs = new Point();
                        Point MidPe = new Point();
                        MidPs.X = Math.Max(startP.X, endP.X) + 10;
                        MidPe.X = MidPs.X;
                        MidPs.Y = startP.Y;
                        MidPe.Y = endP.Y;
                        e.Graphics.DrawLine(pen, startP, MidPs);
                        e.Graphics.DrawLine(pen, MidPs, MidPe);
                        e.Graphics.DrawLine(pen, MidPe, endP);
                    }
                    ...
                    if (sl && el)
                    {
                        Point MidPs = new Point();
                        Point MidPe = new Point();
                        MidPs.X = Math.Min(startP.X, endP.X) - 10;
                        MidPe.X = MidPs.X;
                        MidPs.Y = startP.Y;
                        MidPe.Y = endP.Y;
                        e.Graphics.DrawLine(pen, startP, MidPs);
                    ...
                    if ((sr && el) || (sl && er))
                    {
                        ...
                        int mid = (startP.X + endP.X) / 2;
                        if (newStartP.X == ElementsGUI[link.iElement].Location.X || newStartP.X == ElementsGUI[link.jElement].Location.X)
                        {
                            Point MidPs1 = new Point();
                            Point MidPs2 = new Point();
                            Point MidPe1 = new Point();
                            Point MidPe2 = new Point();
                            MidPs1.X = Math.Min(newStartP.X, newEndP.X) - 10;
                            MidPs1.Y = newStartP.Y;
                            MidPs2.X = MidPs1.X;
                            MidPs2.Y = (newStartP.Y + newEndP.Y) / 2;
                            MidPe1.X = Math.Max(newStartP.X, newEndP.X) + 10;
                            MidPe1.Y = MidPs2.Y;
                            MidPe2.X = MidPe1.X;
                            MidPe2.Y = newEndP.Y;
                            e.Graphics.DrawLine(pen, newStartP, MidPs1);
                            e.Graphics.DrawLine(pen, MidPs1, MidPs2);
                            e.Graphics.DrawLine(pen, MidPs2, MidPe1);
                            e.Graphics.DrawLine(pen, MidPe1, MidPe2);
                            e.Graphics.DrawLine(pen, MidPe2, newEndP);

                        }
                        else
                        {
                            if (Math.Abs(newEndP.X - newStartP.X) >= 20)
                            {
                                Point MidPs = new Point();
                                Point MidPe = new Point();
                                MidPs.X = (newStartP.X + newEndP.X) / 2;
                                MidPe.X = (newStartP.X + newEndP.X) / 2;
                                MidPs.Y = newStartP.Y;
                                MidPe.Y = newEndP.Y;
                                e.Graphics.DrawLine(pen, newStartP, MidPs);
                                e.Graphics.DrawLine(pen, MidPs, MidPe);
                                e.Graphics.DrawLine(pen, MidPe, newEndP);
                            }
                            else
                            {
                                Point MidPs1 = new Point();
        ...
        public void SetGraphPoints(Circuit circuit, List<Link> AllLinks)
        {
            ...
            if (nLinks > 0)
            {
                ...
                AllSameGraph.Add(NewSG);
                
                for (int i = 1; i < nLinks; i++)
                {
                    int SameGraphIndex=-1;
                    bool isFound = false;
                    foreach (SameGraphPoints SG in AllSameGraph)
                    {
                        SameGraphIndex++;
                    ...
                    if (isFound)
                    {
                        AllSameGraph[SameGraphIndex].Points.Add(AllLinks[i].i);
                        AllSameGraph[SameGraphIndex].Points.Add(AllLinks[i].j);
                    }
                    else
                    {
            ...
            while (isChanged)
            {
                ...
                foreach (SameGraphPoints SG in AllSameGraph)
                {
                    ii++;
                    jj = -1;
                    foreach (SameGraphPoints TestSG in AllSameGraph)
                    {
                        jj++;
                        if (ii != jj)
                        {
                ...
                if (isChanged)
                {
                    ...
                    foreach (int SGP in AllSameGraph[Maxij].Points)
                    {
                        AllSameGraph[Minij].Points.Add(SGP);
                    }
                    AllSameGraph.RemoveAt(Maxij);
                }

            }
            ///////////////////////Find same groups /////////////////

            ///////////////////////find number of same groups /////////////////
            int nSame = 0;
            foreach (SameGraphPoints SG in AllSameGraph) nSame++;
            circuit.nGraphPoint = nSame;
            ///////////////////////find number of same groups /////////////////


            ///////////////////////ReArrange same groups /////////////////
            ...
            if (FirstFounded) circuit.AllSameGraph.Add(AllSameGraph[FirstSGInd]);
            int ind=-1;
            foreach (SameGraphPoints SG in AllSameGraph)
            {
                ind++;
                if (ind != FirstSGInd && ind != EndSGInd) circuit.AllSameGraph.Add(SG);
            }
            if (EndFounded) circuit.AllSameGraph.Add(AllSameGraph[EndSGInd]);
            ///////////////////////ReArrange same groups /////////////////
            

            ///////////////////////set elements InputGraph and OutputGraph /////////////////
            ...
            for (int GPoint = 1; GPoint <= 2 * (circuit.nElements - 1); GPoint++)
            {
                ...
                if (!isLink)
                {
                    circuit.isReadyToFit = false;
                    break;
                }
            }

            if (circuit.isReadyToFit)
            {
                int[,] CGraph = new int[circuit.nElements - 2, 3];
                bool[] Visited = new bool[nSame];
                for (int i = 0; i < nSame; i++) Visited[i] = false;
            ...
            //circuit.isReadyToFit = circuit.isReadyToFit;
        }

        public void DFS(int v, int[,] CGraph, int nCGraphLinks, ref bool[] Visited)
        {
            Visited[v - 1] = true;
            for (int i = 0; i < nCGraphLinks; i++)
            {
                if (CGraph[i, 0] == v && Visited[CGraph[i, 1] - 1] == false)
                    DFS(CGraph[i, 1], CGraph, nCGraphLinks, ref Visited);
                if (CGraph[i, 1] == v && Visited[CGraph[i, 0] - 1] == false)
                    DFS(CGraph[i, 0], CGraph, nCGraphLinks, ref Visited);
            }
        }
        ...
        private void TBElementVal1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (ChangingSessionCompleted && ChangingCircuitCompleted) UpdateDiagrams();
        }
        ...
        {
            ClearPlot(chart1);
            if (FitSelectedSession > -1 && Form1.AllSessions[FitSelectedSession].Mode == 0)
            if (Form1.AllSessions[FitSelectedSession].nCircuits > 0 && FitSelectedCircuit>-1)
            {
                if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].isReadyToFit && Form1.AllSessions[FitSelectedSession].isFinished)
                {
                    double ReZ = 0;
                    double ImZ = 0;
                    ...
                    int nData = Form1.AllSessions[FitSelectedSession].ACFrqNStep;
                    double[] ReZTable = new double[nData];
                    double[] ImZTable = new double[nData];

                    //double FrqStep = Form1.AllSessionsData[FitSelectedSession].Frq[1] / Form1.AllSessionsData[FitSelectedSession].Frq[0];
                    double frqMultStep = Math.Pow(Form1.AllSessions[FitSelectedSession].ACFrqTo / Form1.AllSessions[FitSelectedSession].ACFrqFrom, 1.0 / (Form1.AllSessions[FitSelectedSession].ACFrqNStep - 1));
                    for (int ifrq = 0; ifrq < nData; ifrq++)
                    {
                        double frq = Math.Pow(frqMultStep, ifrq) * Form1.AllSessions[FitSelectedSession].ACFrqFrom;
                        graphimpedancereim(ref frq, a, ref ma, ref ReZ, ref ImZ, ref nGraphPoints, CGraphDag, ref nElements);
                        ReZTable[ifrq] = ReZ;
                        ImZTable[ifrq] = ImZ;
                    }

                    double[] dReZTable = new double[nData];
                    double[] dImZTable = new double[nData];
                    for (int ifrq = 0; ifrq < nData; ifrq++)
                    {
                        dReZTable[ifrq] = Form1.AllSessionsData[FitSelectedSession].ReZ[ifrq] / 1000;
                        dImZTable[ifrq] = Form1.AllSessionsData[FitSelectedSession].Imz[ifrq] / 1000;
                    }

                    PlotImpedanceExpFit(chart1, Form1.AllSessionsData[FitSelectedSession].Frq, dReZTable, dImZTable, ReZTable, ImZTable, nData);
                ...
                if (Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].isReadyToFit && !Form1.AllSessions[FitSelectedSession].isFinished)
                {
                    ...
                    //ClearPlot(chart);
                    PlotImpedanceFit(chart1, frqTable, ReZTable, ImZTable, nData);
                    GBFitting.Enabled = false;
                }
                ...
                if (!Form1.AllSessions[FitSelectedSession].Circuits[FitSelectedCircuit].isReadyToFit && Form1.AllSessions[FitSelectedSession].isFinished)
                {
                    ...
                    PlotImpedanceExp(chart1, Form1.AllSessionsData[FitSelectedSession].Frq, Form1.AllSessionsData[FitSelectedSession].ReZ, Form1.AllSessionsData[FitSelectedSession].Imz, nData);
                    GBFitting.Enabled = false;
                }
            }
            else
            {
                if (Form1.AllSessions[FitSelectedSession].isFinished)
                {
                    if (Form1.AllSessions[FitSelectedSession].Mode == 1 || Form1.AllSessions[FitSelectedSession].Mode == 2)
                    {
                        //double FrqStart = Form1.AllSessions[FitSelectedSession].ACFrqFrom;
                        //double FrqTo = Form1.AllSessions[FitSelectedSession].ACFrqTo;
                        ...
                        //PlotImpedanceExp(chart1, frqTable, Form1.AllSessionsData[FitSelectedSession].ReZ, Form1.AllSessionsData[FitSelectedSession].ImZ, nData);
                        GBFitting.Enabled = false;
                    }

                    if (Form1.AllSessions[FitSelectedSession].Mode == 2 || Form1.AllSessions[FitSelectedSession].Mode == 3)
                    {
                        //double VltStart = Form1.AllSessions[FitSelectedSession].IVVoltageFrom;
                        //double VltTo = Form1.AllSessions[FitSelectedSession].IVVoltageTo;
                        //double VltStep = Form1.AllSessions[FitSelectedSession].IVVoltageStep;
                        //int nData = Form1.AllSessions[FitSelectedSession].IVVoltageNStep;
                        //for (double vlt = VltStart; vlt <= VltTo; vlt = vlt + VltStep) nData++;
                        //double[] vltTable = new double[Form1.AllSessionsData[FitSelectedSession].ReceivedDataCount];

                        //int i;

                        //i = -1;
                        //for (double vlt = VltStart; vlt <= VltTo; vlt = vlt + VltStep)
                        //{
                        //    i++;
                        //    vltTable[i] = vlt;
                        //}

                        PlotIV(chart1, Form1.AllSessionsData[FitSelectedSession].Vlt, Form1.AllSessionsData[FitSelectedSession].Amp,
                        Form1.AllSessionsData[FitSelectedSession].ReceivedDataCount, false, "V", "I");
                        GBFitting.Enabled = false;
                    }
                }
            }
        ...
        private void BtnFindFit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //double FrqStep = Form1.AllSessionsData[FitSelectedSession].Frq[1] / Form1.AllSessionsData[FitSelectedSession].Frq[0];
                double frqMultStep = Math.Pow(Form1.AllSessions[FitSelectedSession].ACFrqTo / Form1.AllSessions[FitSelectedSession].ACFrqFrom, 1.0 / (Form1.AllSessions[FitSelectedSession].ACFrqNStep - 1));
                for (double frq = FrqStart; frq <= FrqTo; frq = frq * frqMultStep)
                {
                    k++;
                    FrqTable[k] = frq;

                    AbsZ2[k] = Form1.AllSessionsData[FitSelectedSession].Imz[k] / 1000 * Form1.AllSessionsData[FitSelectedSession].Imz[k] / 1000 +
                        Form1.AllSessionsData[FitSelectedSession].ReZ[k] / 1000 * Form1.AllSessionsData[FitSelectedSession].ReZ[k] / 1000;
                }

                if (!isForbiden) FindFitMethodPolar(ref nData, FrqTable, AbsZ2, InitialValues, ref ma, ia, ref nGraphPoints, CGraphDag, ref nElements, a);
