...
namespace EISProjects
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            CBIVVoltageRangeMode.Items[0] = "[-" + (int)(5 * Settings.AnalogCommon_factor) + " (V) .. " + (int)(5 * Settings.AnalogCommon_factor) + " (V)]";
    ...
    new System.ComponentModel.ComponentResourceManager(typeof(Form1));
        ...
        private void LoadSelectedProperties()
        {
            ...
            CBCharge_DataCount.Value = Convert.ToDecimal(AllSessions[Selected].Charge_DataCount);
            ...
            CBCharge_dt.Value = Convert.ToDecimal(AllSessions[Selected].Charge_dt_ms);
            ...
            {
                CB_PGMode.Enabled = true;
                //CB_PGMode.SelectedIndex = 0;
            }
        ...
        private void CreateNewSession(int Method)
        {
            ...
            if (theFirstOpenMode == -1 && Settings.isChrono) theFirstOpenMode = 5;
            ...
            if (theFirstOpenMode != -1)
            {
                ...
                NewSession.Charge_DataCount = FactoryDefault.Charge_DataCount;
                ...
                NewSession.Charge_dt_ms = FactoryDefault.Charge_dt_ms;
        ...
        {
            if (AllSessions[Selected].Mode == 0)
                DigitTextChange(ref AllSessions[Selected].DCVoltageConstant, TBDCVoltageConstant, -5*Settings.AnalogCommon_factor, 5 * Settings.AnalogCommon_factor);
            else
        ...
        private void TBACFrqFrom_TextChanged(object sender, EventArgs e)
        {
            if (AllSessions[Selected].Mode == 0)
                ...
                DigitTextChange(ref AllSessions[Selected].DCVoltageFrom, TBACFrqFrom, -5 * Settings.AnalogCommon_factor, 5 * Settings.AnalogCommon_factor);
        }
        ...
        private void TBACFrqTo_TextChanged(object sender, EventArgs e)
        {
            if (AllSessions[Selected].Mode == 0)
                ...
                DigitTextChange(ref AllSessions[Selected].DCVoltageTo, TBACFrqTo, -5 * Settings.AnalogCommon_factor, 5 * Settings.AnalogCommon_factor);
        }
        ...
        private void StartRunning()
        {
            ...
            else
            {
                ...
                if (EIS.Connected)
                {
                    ...
                    //moosa
                    if (Settings.AnalogCommon_intOffset > 2300) Settings.AnalogCommon_intOffset = 2106;
                    if (Settings.AnalogCommon_intOffset < 1700) Settings.AnalogCommon_intOffset = 2106;
                    iacdac_zero = Settings.AnalogCommon_intOffset;
                    //moosa
        ...
        private int Dim(Sessions S)
        {
            ...
            if (S.Mode == 5)
            {
                ...
                //    Dimention = S.Chrono_n - 1;
                //else
                Dimention = (int)(S.Charge_TotalTime / S.Charge_dt_ms * 1000) + 1;
            }
            ...
            try
            {
                ...
                if (isPreProccessingCompleted)
                {
                    ...
                    if (AllSessions[EIS.RunningSession].Mode == 5)
                    {
                        ...
                        AllSessionsData[EIS.RunningSession].Imz = new double[Cnt];
        ...
        private void ChronoByCV(int ivltStart, int nThisStep, int iTimeStep)
        {
            ...
            double deltadouble = 0;// ((ivvltto - ivvltfrom) / (AllSessions[EIS.RunningSession].IVVoltageNStepp - 1));
            ...
            deltaint = 0;
            int CVItteration = 1;
            int NumberPerHalfCycle = 1;

            int CVnFirst = (int)((AllSessions[EIS.RunningSession].IVvoltageTo - AllSessions[EIS.RunningSession].CVStartpoint) / (AllSessions[EIS.RunningSession].IVvoltageTo - AllSessions[EIS.RunningSession].IVVoltageFrom) * (AllSessions[EIS.RunningSession].IVVoltageNStepp - 1)) + 1;
            string ans;
            Port.Write("input 00 " + CVItteration.ToString() + WriteReadToChar);
            try
            {
                Thread.Sleep(1);
                ans = Port.ReadTo(ReadToChar);
            ...
            //cvnfirst
            Port.Write("input 02 " + NumberPerHalfCycle.ToString() + WriteReadToChar);
            try
            {
                Thread.Sleep(1);
                ans = Port.ReadTo(ReadToChar);
            ...
            //IVVoltageNStepp
            Port.Write("input 04 " + NumberPerHalfCycle.ToString() + WriteReadToChar);
            try
            {
                Thread.Sleep(1);
                ans = Port.ReadTo(ReadToChar);
            ...
            DebugListBox.Items.Add("stp:" + DebugListBox.Items.Count.ToString() + "input 03 " + ivltStart.ToString());
        ...
        private bool Start_IV_Chrono()
        {
            ...
            try
            {
                ...
                Port.DiscardOutBuffer();
                Port.DiscardInBuffer();
                Port.Write("iacdac " + iacdac_zero.ToString() + WriteReadToChar);
                Thread.Sleep(10);
                ans = Port.ReadTo(ReadToChar);
                current_iacdac = 2047;
               ...
               // Port.Write(WriteReadToChar);
                ...
                if (AllSessions[EIS.RunningSession].Mode == 3)
                {
                    ...
                    else
                    {
                        ...
                        int total_point = (AllSessions[EIS.RunningSession].IVVoltageNStepp) * (AllSessions[EIS.RunningSession].CVItteration) * 2;
                        //if (total_point>10000)
                        if (IVChronoTimeStep > 100)
                            newfd.FigureTimer.Interval = (int)IVChronoTimeStep * 2;
                        else
                            newfd.FigureTimer.Interval = 200;

                        newfd.DensityOfPointsPlots = total_point / 10000 + 1;
                ...
                else if (AllSessions[EIS.RunningSession].Mode == 2)
                {
                    ...
                    if (!AllSessions[EIS.RunningSession].Chrono_isfast)
                    {
                        ...
                        DebugListBox.Items.Add("stp:" + DebugListBox.Items.Count.ToString() + " Number of Steps:" + nThisStep + 1);
                        DebugListBox.Items.Add("stp:" + DebugListBox.Items.Count.ToString() + " Time Step:" + iTimeStep.ToString());
                        ...
                        double total_time =
                        AllSessions[Selected].Chrono_t1 +
                        AllSessions[Selected].Chrono_t2 +
                        AllSessions[Selected].Chrono_t3 +
                        AllSessions[Selected].Chrono_t4 +
                        AllSessions[Selected].Chrono_t5 +
                        AllSessions[Selected].Chrono_t6 +
                        AllSessions[Selected].Chrono_t7 +
                        AllSessions[Selected].Chrono_t8 +
                        AllSessions[Selected].Chrono_t9 +
                        AllSessions[Selected].Chrono_t10;



                        int total_point = (int)(total_time / (IVChronoTimeStep / 1000.0));

                        newfd.DensityOfPointsPlots = total_point / 10000 + 1;

                        if ((IVChronoTimeStep * newfd.DensityOfPointsPlots) > 100)
                            newfd.FigureTimer.Interval = (int)IVChronoTimeStep * 2 * newfd.DensityOfPointsPlots;
                        else
                            newfd.FigureTimer.Interval = 200;
        ...
        private bool Start_Charge()
        {
            ...
            try
            {
                ...
                ans = Port.ReadTo(ReadToChar);
                current_iacdac = 2047;
                SetLabel(ref label_iacdac, iacdac_zero);
                Port.DiscardOutBuffer();
                Port.DiscardInBuffer();
                Port.Write("iIs 3" + WriteReadToChar);
                Thread.Sleep(200);
                ...
                //?????????????????????????????????????????????
                zeroset = Settings.Zeroset1;//felan voltage 1
               ...
               ///////////////////////////////////////////////////////////////////////////////
                ...
                battery.datacount = 0;
                battery.discharge_hit = 0;
                battery.charge_hit = 0;
                battery.adc_hit_number = 2;
                battery.hit_count = 0;
                battery.up_voltage = AllSessions[EIS.RunningSession].Charge_VoltageMax; //auto select voltage
                battery.down_voltage = AllSessions[EIS.RunningSession].Charge_VoltageMin; //auto select voltage   mAx(abs(up),abs(down));
                battery.charge = CurrentCharge; //raw
                battery.discharge = CurrentDischarge; //raw
                battery.charge_current = AllSessions[EIS.RunningSession].Charge_CurrentCharge; //raw
                battery.discharge_current = AllSessions[EIS.RunningSession].Charge_CurrentDischarge; //raw
                battery.measured_adc = 0;
                battery.total_cycle = AllSessions[EIS.RunningSession].Charge_NumberOfCycles;
                ...
                double x2 = 0;
                x1 = 0;
                x2 = AllSessions[EIS.RunningSession].Charge_TotalTime;
                ...
                Port.DiscardOutBuffer(); //Clear Buffer
                Port.DiscardInBuffer(); //Clear Buffer
                Port.Write("ivset " + battery.setpoint.ToString() + ReadToChar);
                //Port.Write(WriteReadToChar);
                ...
                //if (AllSessions[EIS.RunningSession].Chrono_Total_Period > 0)
                {
                    if (!isCharge_TimerTickSet)
                    {
                        ...
                        int interval = 0;
                        interval = (int)(AllSessions[EIS.RunningSession].Charge_dt_ms);
                        if (interval < 1) interval = 1;
        ...
        private void EISInitializeAutoMaximumIDCSelectFinder(ref int Max)
        {
            ...
            Port.DiscardOutBuffer(); //Clear Buffer
            Port.DiscardInBuffer(); //Clear Buffer
            Port.Write("postfilter 3"+WriteReadToChar);
            string ans = Port.ReadTo(ReadToChar);
            Port.DiscardOutBuffer(); //Clear Buffer
            Port.DiscardInBuffer(); //Clear Buffer
        ...
        private void IVTimer_TickProc()
        {
...

            {
                try
                {
                    // for (int ii = 0; ii < nReadBuffer; ii++)
                    {
                        double Vmean = 0;
                        double Imean = 0;
                        if ((AllSessions[EIS.RunningSession].Mode == 2 && (!AllSessions[EIS.RunningSession].Chrono_isfast)) || AllSessions[EIS.RunningSession].Mode == 3)
                    ...
                    try
                    {
                        ...
                        else //if (AllSessions[EIS.RunningSession].isCVEnable) //cv or chrono not fast   (multi cycle)
                        {
                            //finalize
        ...
        public struct battery_struct
        {
            public int datacount;//=0
            public int discharge_hit;//=0
            public int charge_hit;//=0
            public int adc_hit_number;//=0
            public int hit_count;//=0
            public double up_voltage;//max voltage //auto select voltage
            public double down_voltage;//min voltage//auto select voltage   mAx(abs(up),abs(down));
            public int charge;//current + raw
            public int discharge;// current - raw
            public double measured_adc;//= active =0
            public int total_cycle;//cycle number
            public int total_datacount;//data_num
            public int setpoint;

            public double discharge_current { get; internal set; }
            public double charge_current { get; internal set; }
            public int charging { get; internal set; }
        }
        battery_struct battery;

        public static double IMP_RE_MAX=0;
        public static double IMP_IM_MAX=0;
        public static double IMP_RE_MIN=0;
        public static double IMP_IM_MIN=0;
        private static int iacdac_zero=2047;

        private void battery_tick(double measured_voltage)
        {
            //int Set_Point = battery.lastsetpoint;
            if (measured_voltage > battery.up_voltage && (battery.discharge_hit  - battery.charge_hit)<1)
            {
                battery.hit_count += 2;
                if (battery.hit_count > battery.adc_hit_number)
                {
            ...
            else if (measured_voltage < battery.down_voltage && (battery.charge_hit - battery.discharge_hit) < 0)
            {
                battery.hit_count += 2;
                if (battery.hit_count > battery.adc_hit_number)
                {
        ...
        private void battery_timer_tick()
        {
            try
            {
                ...
                double volt = GetDCVConvert(Vmean, 0, AllSessions[EIS.RunningSession].Charge_VoltageRangeMode);
                double current = GetDCIConvert(Imean, AllSessions[EIS.RunningSession].Charge_CurrentRangeMode, 0);
                ...
                AllSessionsData[EIS.RunningSession].Imz[battery.datacount] = battery.charge_current;//fill
                else
                AllSessionsData[EIS.RunningSession].Imz[battery.datacount] = - battery.discharge_current;//fill
                ...
                if (battery.charge_hit < battery.total_cycle && battery.datacount < battery.total_datacount)
                {
                    Port.DiscardOutBuffer(); //Clear Buffer
                    Port.DiscardInBuffer(); //Clear Buffer
                    Port.Write("ivset " + battery.setpoint.ToString() + ReadToChar);
        ...
        private void Charge_Timer_Tick0(object sender, EventArgs e)
        {
                ...
                if (AllowToTick)
                {
                    ...
                    else
                    {
                        ...
                        if (isDataReceived)
                        {
                            ...
                            //moosa
                             SettingsITau_H = (27E-12) * Math.Pow(10, AllSessions[EIS.RunningSession].EISDCCurrentRangeModea);
                            ...
                            if (cnt==0 && DigitalEISStepUnSuccessCounter > 12) { isIvalidtofit = true; isVvalidtofit = true; }
                            if (cnt > 0 && DigitalEISStepUnSuccessCounter > 6) { isIvalidtofit = true; isVvalidtofit = true; }
                                ...
                                (isIvalidtofit && isVvalidtofit && (AllSessions[EIS.RunningSession].EISACRegulatorMode == 1)))
                            {
                                    ...
                                    (EISAverageNumberCounter >= AllSessions[EIS.RunningSession].EISAverageNumberH && (Frequency > 10.0)))
                                {
                                    ...
                                    if (IMP_RE_MAX < EISRealImpMean)
                                        IMP_RE_MAX = EISRealImpMean;
                                    if (IMP_IM_MAX < EISImagImpMean)
                                        IMP_IM_MAX = EISImagImpMean;

                                    if (IMP_RE_MIN > EISRealImpMean)
                                        IMP_RE_MIN = EISRealImpMean;
                                    if (IMP_IM_MIN > EISImagImpMean)
                                        IMP_IM_MIN = EISImagImpMean;
                            ...
                            {//validity_I !(isThisTheBestImlp>=2) &&
                                if (!isIvalidtofit)
                                {
                                    ...
                                    if (isINeedToAmp)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (checkBox_HImpede.Checked && (Frequency < (double)(numericUpDown_freq.Value)))
                                            {
                                                IDCSelectMax = 7;
                                            }
                                ...
                                if (DigitalEISStepUnSuccessCounter > 20) StopRun();
                            }
        ...
        private void mottnextvoltage(double volt, ref double TheVoltageisset, ref double current)
        {
            ...
            iselect = AllSessions[EIS.RunningSession].EISDCCurrentRangeModea;
            {
                // try
                {
                    ...
                    Thread.Sleep(250);
                    ans = Port.ReadTo(ReadToChar);
                    SetLabel(ref label_iacdac, (int)iacdac);
                    //Izero(0);
                    /*   Port.Write("aczero" + WriteReadToChar);
                       Thread.Sleep(100);
                       ans = Port.ReadTo(ReadToChar);
        ...
        void iaczero_to_iaczerodac(double i_zero, double domain5, ref int iis, ref int izdac)
        {
            ...
            izdac = (int)(i_zero * domain + Settings.AnalogCommon_intOffset);
        ...
        void iaczerodac_to_iaczero(ref double i_zero, double domain5, int iis, int izdac, double V_O4)
        {
            double R;
            double v_i_null = 0;
            v_i_null = (izdac - Settings.AnalogCommon_intOffset) / 2048.0 * 5.0;
            if (iis == 3)
            {
        ...
        private void Izero(double i, int Irange)
        {
            ...
            label_idcmA.Text = (i*1000).ToString("0.0000mA");
            double V_O4 = 0;
            if (Irange == 0)
                V_O4 = i * 1;
            if (Irange == 1)
            ...
            label_idcraw.Text = (current_iz*1000).ToString("0.0000mA");
           ...
           // label_iacdac.Text += " " + i.ToString(0.000) + "|" + i_zero;
            //Port.DiscardOutBuffer();
            Port.DiscardInBuffer();
            Port.Write("iIs " + iIs.ToString() + WriteReadToChar);
            Thread.Sleep(100);
        ...
        private void IZsetoffset()
        {
            ...
            ans = Port.ReadTo(ReadToChar);
            Port.DiscardInBuffer();
            Port.Write("iacdac 2106" + WriteReadToChar);
            ans = Port.ReadTo(ReadToChar);
            SetLabel(ref label_iacdac, (int)2047);
            current_iacdac = 2047;
            // label_iacdac.Text += " " + i.ToString(0.000) + "|" + i_zero;
            //Port.DiscardOutBuffer();
            Port.DiscardInBuffer();
            Port.Write("iIs 1" + WriteReadToChar);
            ans = Port.ReadTo(ReadToChar);
            SetLabel(ref label_iIs, iIs);
            Thread.Sleep(10);
            Thread.Sleep(100);
            Port.DiscardInBuffer();
            Port.Write("idcselect 3" + WriteReadToChar);
            ans = Port.ReadTo(ReadToChar);
            Thread.Sleep(100);
            Port.DiscardOutBuffer(); //Clear Buffer
            Port.DiscardInBuffer(); //Clear Buffer
            Port.Write("ivset 2047" + WriteReadToChar);
            ...
            try
            {
                ...
                //double TheVoltageisset = GetDCVConvert(Vmean, DEISCurrentVmlp, AllSessions[EIS.RunningSession].EISVoltageRangeMode);
                //DebugListBox.Items.Add("stp:" + DebugListBox.Items.Count.ToString() + " The Voltage is set:" + TheVoltageisset.ToString());
                Imean -= Settings.GetDCI_Offset3d;
                // Imean =GetDCIConvert(Imean, 4, 0);
                Imean=Imean;
                //DebugListBox.Items.Add("stp:" + DebugListBox.Items.Count.ToString() + " The Current is set:" + TheCurrentisset.ToString());
        ...
        public static int SetDCVConvert(double VltOrCurrentInGalvanostat, int VSelect, int ISelect, int PGMode)
        {
            ...
            if (PGMode != 3)
            {
                ...
                //backward iv8
                if(Settings.ACMultFactor_Select0 > 0.25)
                if(ISelect < 2 && VSelect == 0)
                {
                    vlt = VltOrCurrentInGalvanostat;
                    vlt = vlt / Settings.SetDCV_Select1 / 5.55;
                }
        ...
        public static double Inverse_SetDCVConvert_dV(int VltOrCurrentInGalvanostat_Int, int VSelect, int ISelect, int PGMode)
        {
            ...
            if (PGMode != 3)
            {
                ...
                double _vlt = (ivlt - Settings.SetDCV_Offset) * Settings.SetDCV_Domain / 2047;

                if (VSelect == 0)
                    vlt = _vlt * Settings.SetDCV_Select0;
                else
                    vlt = _vlt * Settings.SetDCV_Select1;

                if (ISelect < 2 && VSelect == 0)
                {
                    
                    vlt = _vlt * Settings.SetDCV_Select1 * 5.55;
                }
        ...
        {
            double domain = Settings.GetDCV_Domain * Math.Pow(5.55* Form1.Settings.ACMultFactor_Select0, 1 - SetSelect);
        ...
        private void ReadAllDataFromFile(string FileName)
        {
            try
            {
                ...
                for (int s = StartIndex; s < StartIndex + nSessions; s++)
                {
                    ...
                    if (Version >= 1025) NewSession.Charge_DataCount = br.ReadInt32(); else NewSession.Charge_DataCount = FactoryDefault.Charge_DataCount;
                    ...
                    if (Version >= 1025) NewSession.Charge_dt_ms = br.ReadInt32(); else NewSession.Charge_dt_ms = FactoryDefault.Charge_dt_ms;
        ...
        private void CBMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            if ((CBMode.SelectedIndex == 0 && Settings.isEIS) || (CBMode.SelectedIndex == 1 && Settings.isMSH) ||
                ...
                (CBMode.SelectedIndex == 4 && Settings.isPulse) || (CBMode.SelectedIndex == 5 && Settings.isChrono))
            {
        ...
        private void Duplicate(int Index)
        {
            ...
            NewSession.Charge_DataCount = AllSessions[Index].Charge_DataCount;
            ...
            NewSession.Charge_dt_ms = AllSessions[Index].Charge_dt_ms;
        ...
        {
            if (!(AllSessions[Selected].PGmode == 3))
                DigitTextChange(ref AllSessions[Selected].IVVoltageFrom, TBIVVoltageFrom, -5 * Settings.AnalogCommon_factor, 5 * Settings.AnalogCommon_factor);
            else
                DigitTextChange(ref AllSessions[Selected].IVVoltageFrom, TBIVVoltageFrom, -5000000 * Settings.AnalogCommon_factor, 5000000 * Settings.AnalogCommon_factor);
            TBIVVelosity_Validated(null, null);
            TBIVVoltageFrom.Text = AllSessions[Selected].IVVoltageFrom.ToString();
        ...
        private void TBIVVoltageTo_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                if (!(AllSessions[Selected].PGmode == 3))
                    DigitTextChange(ref AllSessions[Selected].IVvoltageTo, TBIVVoltageTo, -5 * Settings.AnalogCommon_factor, 5 * Settings.AnalogCommon_factor);
                else
                    DigitTextChange(ref AllSessions[Selected].IVvoltageTo, TBIVVoltageTo, -5000000 * Settings.AnalogCommon_factor, 5000000 * Settings.AnalogCommon_factor);
            }
        ...
        private void BtnVoltageOffsetRemoval_Click(object sender, EventArgs e)
        {
            ...
            if (EIS.Connected)
            {
                ...
                try
                {
                    ...
                    //////////////////////////////////////////////////////////////////////////////////////
                    //moosa Izero dac offset find
                     Ioffset = 0;
                     Voffset = 0;
                    double IO2, IO1;
                    Port.DiscardOutBuffer();
                    Port.DiscardInBuffer();
                    Port.Write("iacdac 1847" + WriteReadToChar);
                    Thread.Sleep(10);
                    ans = Port.ReadTo(ReadToChar);
                    current_iacdac = 1947;
                    SetLabel(ref label_iacdac, 1947);
                    Port.DiscardOutBuffer();
                    Port.DiscardInBuffer();
                    Port.Write("iIs 1" + WriteReadToChar);
                    Thread.Sleep(10);
                    ans = Port.ReadTo(ReadToChar);
                    iIs = 1;
                    SetLabel(ref label_iIs, 1);
                    Port.DiscardOutBuffer(); //Clear Buffer
                    Port.DiscardInBuffer(); //Clear Buffer
                    CalculateSpecificIVOffsets_dumoff(3, 0, 0, ref Ioffset, ref Voffset);
                    IO1 = Ioffset;
                    DebugListBox.Items.Add("stp:" + DebugListBox.Items.Count.ToString() + "isel=3" + " Ioffset=" + Ioffset.ToString() + " done.");
                    //IZsetoffset();
                    Port.DiscardOutBuffer();
                    Port.DiscardInBuffer();
                    Port.Write("iacdac 2247" + WriteReadToChar);
                    Thread.Sleep(10);
                    ans = Port.ReadTo(ReadToChar);
                    Port.DiscardOutBuffer(); //Clear Buffer
                    Port.DiscardInBuffer(); //Clear Buffer
                    ...
                    IO2 = Ioffset;
                    DebugListBox.Items.Add("stp:" + DebugListBox.Items.Count.ToString() + "isel=3" + " Ioffset=" + Ioffset.ToString() + " done.");
                    double M = 400.0 / (IO2 - IO1);
                    Settings.AnalogCommon_intOffset = (int)((Settings.GetDCI_Offset3d - IO1) * M + 1847);
                    if (Settings.AnalogCommon_intOffset > 2300) Settings.AnalogCommon_intOffset = 2106;
                    if (Settings.AnalogCommon_intOffset < 1700) Settings.AnalogCommon_intOffset = 2106;
                    iacdac_zero = Settings.AnalogCommon_intOffset;
                    if (isDummyOn)
        ...
        {
            IntDigitTextChange(ref AllSessions[Selected].CVItteration, TB_CV_Itteration, 1, 100);
        }
        ...
        {
            DigitTextChange(ref AllSessions[Selected].PretreatmentVoltage, TBPretreatmentVoltage, -5 * Settings.AnalogCommon_factor, 5 * Settings.AnalogCommon_factor);
        }
        ...
        private void CBCharge_VoltageRangeMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (isCBCharge_VoltageRangeCompleted)
            {
                ...
                {
                    Charge_MaxVoltage = 5 * Settings.AnalogCommon_factor;
                    Charge_MinVoltage = -5 * Settings.AnalogCommon_factor;
                }
                else
                {
                    Charge_MaxVoltage = 1;
        ...
        private void CBCharge_TotalTime_ValueChanged(object sender, EventArgs e)
        {
            Evaluate_ChargeTotalTime();
        }

        private void Evaluate_ChargeTotalTime()
        {
            double totalT = Convert.ToDouble(CBCharge_TotalTime.Value);
            double dt = Convert.ToDouble(CBCharge_dt.Value);
            int Cnt = (int)(totalT / dt * 1000) + 1;
            int maxcnt = 10000;
            if (Cnt > maxcnt)
            {
                int newCnt = maxcnt;
                double newdt = totalT * 1000/ (newCnt - 1);
                CBCharge_dt.Value = Convert.ToDecimal(Convert.ToInt32(Math.Round(newdt)));
                AllSessions[Selected].Charge_TotalTime = totalT;
            }
            else
            {
                AllSessions[Selected].Charge_dt_ms = Convert.ToInt32(CBCharge_dt.Value);
                AllSessions[Selected].Charge_TotalTime = Convert.ToDouble(CBCharge_TotalTime.Value);
            }
        }
        ...
        {
            AllSessions[Selected].Charge_NumberOfCycles = Convert.ToInt32(CBCharge_NumberOfCycles.Value);
        }

        private void CBCharge_dt_ValueChanged(object sender, EventArgs e)
        {
            Evaluate_ChargeTotalTime();
        }
